...
use swc_core::ecma::ast::{
    ...
    BlockStmt, BlockStmtOrExpr, Decl, Expr, Function, Ident, IfStmt, MemberExpr,
...
fn process_pattern_b_params(params: &mut Vec<Param>, body: &mut BlockStmt) {
    ...
    for (stmt_idx, stmt) in body.stmts.iter().enumerate() {
        if let Some((param_idx, param_name, default_val)) = extract_arguments_default(stmt) {
            // Ensure we have enough params, padding with placeholders if needed
            while params.len() <= param_idx {
                let placeholder_name = format!("_param_{}", params.len());
                params.push(make_ident_param(placeholder_name.into()));
            }

            // Only replace if the current param at that index is a plain ident
            // (not already an assignment pattern or something else)
            match &params[param_idx].pat {
                Pat::Ident(_) => {
                    // Replace with assignment pattern using the var name
                    params[param_idx].pat = Pat::Assign(AssignPat {
                        span: DUMMY_SP,
                        left: Box::new(Pat::Ident(BindingIdent {
                            id: Ident::new_no_ctxt(param_name, DUMMY_SP),
                            type_ann: None,
                        })),
                        right: default_val,
                    });
                ...
                Pat::Invalid(_) => {
                    // Was a placeholder we just added or a gap
                    params[param_idx].pat = Pat::Assign(AssignPat {
                        span: DUMMY_SP,
                        left: Box::new(Pat::Ident(BindingIdent {
                            id: Ident::new_no_ctxt(param_name, DUMMY_SP),
                ...
                _ => {
                    // Already has a pattern (e.g. already processed), skip
                }
            }
        }
    }
...
fn extract_arguments_default(stmt: &Stmt) -> Option<(usize, Atom, Box<Expr>)> {
    ...
    // Check if it's `arguments.length > N ? arguments[N] : undefined`
    // (no default - just optional param, no assignment pattern needed)
    if let Some(n) = extract_simple_arguments_optional(init) {
        // `var b = arguments.length > N ? arguments[N] : undefined`
        // This is just an optional param, pad but don't add default
        let _ = n; // We don't need to transform this - it becomes just a param
        return None;
    }
    ...
    // Extract N from test: `arguments.length > N && arguments[N] !== undefined`
    let param_idx = extract_arguments_cond_index(&cond.test)?;

    // Consequent must be `arguments[N]`
    if !is_arguments_idx(&cond.cons, param_idx) {
        return None;
    }

    // Alternate must not be undefined (that's the default value)
    if is_void0_or_undefined(&cond.alt) {
        return None;
    }

    Some((param_idx, var_ident.sym.clone(), cond.alt.clone()))
}
...
fn extract_simple_arguments_optional(expr: &Expr) -> Option<usize> {
    ...
    let Expr::Bin(BinExpr { op: BinaryOp::Gt, left, right, .. }) = cond.test.as_ref() else {
        return None;
    };
    if !is_arguments_length(left) {
        return None;
    }
    let n = extract_num_literal(right)?;
...
fn extract_arguments_cond_index(expr: &Expr) -> Option<usize> {
    ...
    if *op == BinaryOp::LogicalAnd {
        let Expr::Bin(BinExpr { op: BinaryOp::Gt, left: ll, right: lr, .. }) = left.as_ref() else {
            return None;
        };
        if !is_arguments_length(ll) {
            return None;
        }
        let n = extract_num_literal(lr)?;
