...
impl VisitMut for Exponent {
    fn visit_mut_expr(&mut self, expr: &mut Expr) {
        ...
        let Expr::Member(member) = callee_expr.as_ref() else { return; };
        let Expr::Ident(obj_ident) = member.obj.as_ref() else { return; };
        if obj_ident.sym != "Math" {
            return;
        }
        let MemberProp::Ident(prop_ident) = &member.prop else { return; };
        if prop_ident.sym != "pow" {
            return;
        }

        // Take ownership and build the ** expression
        let Expr::Call(mut call_owned) =
            std::mem::replace(expr, Expr::Invalid(Default::default()))
        else {
            unreachable!()
        };
