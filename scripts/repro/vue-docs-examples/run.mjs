...
function main() {
  ...
  const rows = fixtures.map((fixture) => runFixture({
    fixture,
    compiler,
    wakaru,
    generatedRoot,
  ...
  const report = { docs_commit: docsCommit, vue_version: vueVersion, rows };
...
function parseArgs(args) {
  const options = { docsRoot: undefined, filter: "", json: false, skipWakaruBuild: false };
...
function runFixture({ fixture, compiler, wakaru, generatedRoot, recoveredRoot }) {
  const generated = compileSfc(fixture.source, fixture.filename, compiler);
  const generatedPath = join(generatedRoot, `${fixture.name}.js`);
  const recoveredPath = join(recoveredRoot, `${fixture.name}.vue`);
...
function compileSfc(source, filename, compiler) {
  ...
  const compiledScript = compiler.compileScript(descriptor, {
    id: `data-v-docs-${filename.replace(/\W/g, "-")}`,
...
function formatReport(report) {
  ...
  const lines = [
    ...
    "",
    "| component | recovered | valid | script setup | imports | template | leaks |",
    "|---|---:|---:|---:|---:|---:|---|",
  ];
  for (const row of report.rows) {
    lines.push(`| ${row.name} | ${yes(row.recovered)} | ${yes(row.parse_ok && row.template_ok)} | ${yes(row.script_setup)} | ${row.missing_imports.length ? row.missing_imports.join(", ") : "yes"} | ${yes(row.template_equivalent)} | ${row.leaked_markers.join(", ")} |`);
  }
  return `${lines.join("\n")}\n`;
}
