...
/// Function-level directives are intentionally outside this finalization pass
/// and remain available to every rewrite rule.
pub(super) fn strip_redundant_module_use_strict(
    module: &mut Module,
    filename: &str,
    bare_imports_are_stable: bool,
) {
    if !module.body.is_empty() && module.body.iter().all(is_use_strict_item) {
        module.body.clear();
        return;
    }
    ...
    module.body.retain(|item| !is_use_strict_item(item));
}
