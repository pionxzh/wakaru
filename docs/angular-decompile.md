...
shared property relationship survives Closure property renaming. The ordinary
three-parameter runtime form, the six-parameter fallback-template form, and
Closure's two-parameter specialization produced by removing unused fallback
arguments are all supported; an unpaired helper with the same call count
remains unknown.
