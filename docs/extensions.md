# wakaru extensions over original wakaru

This document covers cases where wakaru handles more patterns than the
...
wakaru accepts any expression as the super class — plain identifiers,
...
wakaru recognises all three inline forms and converts the result to:
...
wakaru adds a pre-pass (`inline_module_arrow_wrappers`) that specifically
...
would produce `(() => r).a` which is `undefined` at runtime.  wakaru
...
// wakaru output — correct: a depends on n, so both stay in the for-init
for (let n = 10, a = new Array(n), i = 0; i < n; i++) { ... }
```

wakaru uses a fixpoint iteration to compute the full transitive closure of
...
The webpack4 unpacker in wakaru adds two post-extraction rewrite passes
