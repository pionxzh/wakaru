...
2. The full core suite — it covers the pipeline and unpack snapshot tests for
   every supported bundler format and producer variant (webpack4/5, Vercel
   ncc, browserify, SystemJS, esbuild, AMD, Rollup, multi-file), which is
   broader than any hand-picked subset:
