...
- `closure_module_manager_unpack.rs` -- Closure ModuleManager/gstatic structural,
  ...
  chunk tests.
- `angular_ivy_generated.rs` -- Pinned Angular CLI production chunks, their
  Closure `SIMPLE` derivative, and a retained-root Closure `ADVANCED`
  derivative generated from the same application.
